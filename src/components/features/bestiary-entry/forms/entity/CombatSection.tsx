...
import { calculateAbilityModifier, SKILL_ABILITIES } from "@/lib/dnd/calculations";
...
const SAVING_THROW_KEYS: readonly Attribute[] = [
  "strength", "dexterity", "constitution", "intelligence", "wisdom", "charisma",
] as const;

export const SavingThrowsSection: React.FC = () => {
  const { watch, setValue } = useFormContext<Entity>();
  const statBlock = watch("statBlock");
  const proficiency = watch("proficiencyBonus") ?? 2;
  const watchedSavingThrows = watch("savingThrows");
  ...
  React.useEffect(() => {
    const synced = syncSelectedBonuses(savingThrows, (key) =>
      SAVING_THROW_KEYS.includes(key as Attribute)
        ? getProficientBonus(statBlock, key as Attribute, proficiency)
        : null
    );

    if (synced !== savingThrows) {
      setValue("savingThrows", synced, { shouldDirty: true });
    }
        ...
        {SAVING_THROW_KEYS.map((attr) => {
          ...
          return (
            <div
              ...
              onClick={() => toggle(attr)}
              onKeyDown={(e) => { if (e.key === " " || e.key === "Enter") toggle(attr); }}
            >
              <div className="flex items-center gap-2 pointer-events-none">
                <Checkbox checked={isProficient} variant="ghost" size="sm" tabIndex={-1} />
        ...
        {COMMON_SKILLS.map((skill) => {
          ...
          return (
            <div
              ...
              onKeyDown={(e) => { if (e.key === " " || e.key === "Enter") toggle(skill); }}
            >
